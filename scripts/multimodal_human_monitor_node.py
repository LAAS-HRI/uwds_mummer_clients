                ...
                for i, person in enumerate(person_msg.data):
                    ...
                    if person.is_identified > MIN_NB_DETECTION:
                        try:
                            ...
                            # person_msg.header.stamp
                            msg = self.tf_buffer.lookup_transform(self.reference_frame, person_msg.header.frame_id, person_msg.header.stamp)
                            ...
                            if "face" not in self.human_bodies[human_id]:
                                ...
                                if "face" in self.human_meshes:
                                    new_node.properties["mesh_ids"] = self.human_meshes["face"]
                                else:
                                    new_node.properties["mesh_ids"] = []
                                if "face" in self.human_aabb:
